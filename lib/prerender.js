...
prerender.javascriptToExecuteOnPage = function() {
    try {
        ...
        if (html && html[0]) {
            ...
            try {
              jquery_inactive = jQuery.active == 0;
              angular_inactive = angular.element($("html")).scope().$apply(function(scope) {return scope.isAjaxActive()}) == false;
              application_controller_present = $("html").attr("ng-controller") !== undefined;
              isAjaxComplete = jquery_inactive && (!application_controller_present || angular_inactive);
            }
            catch(e) {
              isAjaxComplete = false;
            }
            window.prerenderReady = isAjaxComplete;
